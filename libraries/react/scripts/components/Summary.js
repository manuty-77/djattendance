...
const Summary = (p) => {
  ...
  let unexcused_tardies = eventsWithRolls.filter(esr => (esr.event.roll.status === 'T' || esr.event.roll.status === 'U' || esr.event.roll.status === 'L') && esr.event.status.slip !== 'approved')
  let excused_absences = eventsWithRolls.filter(esr => esr.event.roll.status === 'A' && esr.event.status.slip === 'approved')
  let excused_tardies = eventsWithRolls.filter(esr => (esr.event.roll.status === 'T' || esr.event.roll.status === 'U' || esr.event.roll.status === 'L') && esr.event.status.slip === 'approved')
  const TARDY_LIMIT = 4
