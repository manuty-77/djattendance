...
def generate_menu(context):
  ...
  grad_menu = MenuItem(
      ...
      trainee_only=[SubMenuItem(name='Invites & DVDs', url=f.get_absolute_url()) if f.name == 'Misc' else SubMenuItem(name=f.name, url=f.get_absolute_url()) for f in grad_forms(user)]
  )
