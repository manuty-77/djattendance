...
def generate_menu(context):
  ...
  grad_menu = MenuItem(
      name="Grad",
      common=[SubMenuItem(name='Invites & DVDs', url=f.get_absolute_url()) if f.name_of_model == 'Misc' else SubMenuItem(name=f.name_of_model, url=f.get_absolute_url()) for f in grad_forms(user)],
      specific=[
